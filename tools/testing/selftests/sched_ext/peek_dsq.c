...
static enum scx_test_status run(void *ctx)
{
	...
	pid_count = print_observed_pids(skel->maps.observed_global_pids,
					skel->data->max_samples, "global");
	pid_count += print_observed_pids(skel->maps.observed_local_pids,
					skel->data->max_cpus, "local");

	if (skel->data->debug_ksym_exists && pid_count == 0) {
		printf("✗ Builtins failed: no non-null peeks on builtin dsqs\n");
		failed = true;
	}
	if (skel->data->debug_ksym_exists && pid_count > 0)
		printf("✓ Builtin success: observed non-null peeks\n");
